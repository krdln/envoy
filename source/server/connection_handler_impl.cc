...
ConnectionHandlerImpl::ActiveTcpListener::ActiveTcpListener(ConnectionHandlerImpl& parent,
    ...
    : ConnectionHandlerImpl::ActiveListenerImplBase(parent, std::move(listener), config),
      parent_(parent) {
  if (config.connectionBalancer().has_value()) {
    config.connectionBalancer().value().get().registerHandler(*this);
  }
}

ConnectionHandlerImpl::ActiveTcpListener::~ActiveTcpListener() {
  if (config_.connectionBalancer().has_value()) {
    config_.connectionBalancer().value().get().unregisterHandler(*this);
  }
...
void ConnectionHandlerImpl::ActiveTcpListener::onAcceptWorker(
    Network::ConnectionSocketPtr&& socket, bool hand_off_restored_destination_connections,
    bool rebalanced) {
  if (!rebalanced && config_.connectionBalancer().has_value() &&
      Network::ConnectionBalancer::BalanceConnectionResult::Rebalanced ==
          config_.connectionBalancer().value().get().balanceConnection(std::move(socket), *this)) {
    return;
  }
...
void ConnectionHandlerImpl::ActiveTcpListener::post(Network::ConnectionSocketPtr&& socket) {
  ...
  parent_.dispatcher_.post([socket_to_rebalance, tag = tag(), &parent = parent_]() {
    ...
    dynamic_cast<ActiveTcpListener*>(listener_it->second.get())
        ->onAcceptWorker(std::move(socket_to_rebalance->socket), false, true);
  });
}
